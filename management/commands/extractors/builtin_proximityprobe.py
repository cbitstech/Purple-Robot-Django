...
CREATE_PROBE_TABLE_SQL = 'CREATE TABLE builtin_proximityprobe(id SERIAL PRIMARY KEY, user_id TEXT, guid TEXT, timestamp DOUBLE PRECISION, utc_logged TIMESTAMP, sensor_vendor TEXT, sensor_name TEXT, sensor_power DOUBLE PRECISION, sensor_type BIGINT, sensor_version BIGINT, sensor_resolution DOUBLE PRECISION, sensor_maximum_range DOUBLE PRECISION);'
CREATE_PROBE_USER_ID_INDEX = 'CREATE INDEX ON builtin_proximityprobe(user_id);'
CREATE_PROBE_GUID_INDEX = 'CREATE INDEX ON builtin_proximityprobe(guid);'
CREATE_PROBE_UTC_LOGGED_INDEX = 'CREATE INDEX ON builtin_proximityprobe(utc_logged);'

CREATE_READING_TABLE_SQL = 'CREATE TABLE builtin_proximityprobe_reading(id SERIAL PRIMARY KEY, user_id TEXT, reading_id BIGINT, utc_logged TIMESTAMP, sensor_timestamp DOUBLE PRECISION, sensor_timestamp_utc TIMESTAMP, event_timestamp DOUBLE PRECISION, event_timestamp_utc TIMESTAMP, normalized_timestamp DOUBLE PRECISION, normalized_timestamp_utc TIMESTAMP, distance DOUBLE PRECISION, accuracy DOUBLE PRECISION);'
CREATE_READING_USER_ID_INDEX = 'CREATE INDEX ON builtin_proximityprobe_reading(user_id);'
CREATE_READING_READING_ID_INDEX = 'CREATE INDEX ON builtin_proximityprobe_reading(reading_id);'
CREATE_READING_UTC_LOGGED_INDEX = 'CREATE INDEX ON builtin_proximityprobe_reading(utc_logged);'
...
def exists(connection_str, user_id, reading):
    ...
    cursor.execute('SELECT id FROM builtin_proximityprobe WHERE (user_id = %s AND guid = %s);', (user_id, reading['GUID']))
...
def probe_table_exists(conn):
    cursor = conn.cursor()
    cursor.execute('SELECT table_name FROM information_schema.tables WHERE (table_schema = \'public\' AND table_name = \'builtin_proximityprobe\')')
...
def reading_table_exists(conn):
    cursor = conn.cursor()
    cursor.execute('SELECT table_name FROM information_schema.tables WHERE (table_schema = \'public\' AND table_name = \'builtin_proximityprobe_reading\')')
...
def insert(connection_str, user_id, reading, check_exists=True):
    ...
    reading_cmd = 'INSERT INTO builtin_proximityprobe(user_id, ' + \
                                                     'guid, ' + \
                                                     'timestamp, ' + \
                                                     'utc_logged, ' + \
                                                     'sensor_vendor, ' + \
    ...
    for row in cursor.fetchall():
        ...
        for i in range(0, readings_len):
            reading_cmd = 'INSERT INTO builtin_proximityprobe_reading(user_id, ' + \
                                                                     'reading_id, ' + \
                                                                     'utc_logged, ' + \
                                                                     'event_timestamp, ' + \
                                                                     'event_timestamp_utc, ' + \
            ...
            values.append(reading['DISTANCE'][i])
